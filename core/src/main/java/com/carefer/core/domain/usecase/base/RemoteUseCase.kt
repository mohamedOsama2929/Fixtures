...
abstract class RemoteUseCase<P, R, FR>(protected val errorUtil: CloudErrorMapper) {
    ...
    fun executeApiState(parameters: P, block: CompletionBlock<ApiState<FR>>) {
    ...
    fun executeDownload(parameters: P, block: CompletionBlock<ApiState<FR>>) {
        val response = Request<ApiState<FR>>().apply { block() }
        unsubscribe()
        parentJob = Job()
        CoroutineScope(foregroundContext + parentJob).launch {
            ...
            try {
                ...
                try {
                    val remoteResponseMessage = getMesageFromDownloadResponse(dto = result)

                    if (remoteResponseMessage.isNotEmpty()) {
                        response(false)
                        response(ErrorModel(ErrorStatus.CANNOT_DOWNLOAD_FILE))
                        return@launch
                    } else
                        response(ApiState.Success(convert(result)))
                    response(ApiState.Success(convert(result)))
    ...
    private fun <R> getMesageFromDownloadResponse(dto: R): String {
        val response = dto as Response<*>
    ...
    class Request<T> {
        ...
        private var onError: ((com.carefer.core.domain.entities.base.ErrorModel) -> Unit)? = null
        ...
        fun onError(block: (com.carefer.core.domain.entities.base.ErrorModel) -> Unit) {
            onError = block
        }
        ...
        operator fun invoke(error: com.carefer.core.domain.entities.base.ErrorModel) {
